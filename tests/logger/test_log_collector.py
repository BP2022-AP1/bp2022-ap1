...
class TestLogCollector:
    ...
    def _departure_arrival_1_df(self):
        ...
        departure_arrival_df['arrival_tick'] = departure_arrival_df['arrival_tick'].astype('Int64')
        departure_arrival_df['departure_tick'] = departure_arrival_df['departure_tick'].astype('Int64')
        return departure_arrival_df

    @pytest.fixture
    def _departure_arrival_all_1_df(self):
        departure_arrival_df = pd.DataFrame(
            [["station_1", 10, 20, 'ice_1'], ["station_2", 30, 40, 'ice_1'], ["station_3", 50, 60, 'ice_1']],
    ...
    def _platform_blocked_fault_configuration(self):
        return PlatformBlockedFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="station_1")

    @pytest.fixture
    def _track_blocked_fault_configuration(self):
        return TrackBlockedFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="section_1")

    @pytest.fixture
    def _track_speed_limit_fault_configuration(self):
        return TrackSpeedLimitFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="section_1",
                                                        new_speed_limit=10)

    @pytest.fixture
    def _schedule_blocked_fault_configuration(self):
        return ScheduleBlockedFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="ice_1")

    @pytest.fixture
    def _train_prio_fault_configuration(self):
        return TrainPrioFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="ice_1", new_prio=1)

    @pytest.fixture
    def _train_speed_fault_configuration(self):
        return TrainSpeedFaultConfiguration.create(start_tick=10, end_tick=20, affected_element_id="ice_1",
                                                   new_speed=10)
        ...
        return pd.DataFrame(
            {
                ...
                'fault_type': ['platform_blocked', 'track_blocked', 'track_speed_limit', 'schedule_blocked',
                               'train_prio', 'train_speed'],
                'fault_id': [None, None, None, None, None, None],
                'affected_element': ['station_1', 'section_1', 'section_1', 'ice_1', 'ice_1', 'ice_1'],
    ...
    def setup_faults(self, logger: Logger,
        ...
        logger.inject_platform_blocked_fault(10, _platform_blocked_fault_configuration, "station_1")
        logger.resolve_platform_blocked_fault(20, _platform_blocked_fault_configuration)

        logger.inject_track_blocked_fault(10, _track_blocked_fault_configuration, "section_1")
        logger.resolve_track_blocked_fault(20, _track_blocked_fault_configuration)

        logger.inject_track_speed_limit_fault(10, _track_speed_limit_fault_configuration, "section_1", "100", "10")
        logger.resolve_track_speed_limit_fault(20, _track_speed_limit_fault_configuration)

        logger.inject_schedule_blocked_fault(10, _schedule_blocked_fault_configuration, "ice_1")
        logger.resolve_schedule_blocked_fault(20, _schedule_blocked_fault_configuration)

        logger.inject_train_prio_fault(10, _train_prio_fault_configuration, "ice_1", "2", "1")
        logger.resolve_train_prio_fault(20, _train_prio_fault_configuration)

        logger.inject_train_speed_fault(10, _train_speed_fault_configuration, "ice_1", "100", "10")
    ...
    def test_get_simulation_config_ids(self, _config_ids, logger, log_collector: LogCollector):
        self.setup_departure_arrival_1(logger)
        self.setup_departure_arrival_2(logger)
        self.setup_departure_arrival_3(logger)
        self.setup_departure_arrival_4(logger)
    ...
    def test_departure_arrival_empty(self, logger: Logger, log_collector: LogCollector):
        assert_frame_equal(
            log_collector.get_departures_arrivals_all_trains(logger.run_id),
            pd.DataFrame(columns=['train_id', 'station_id', 'arrival_tick', 'departure_tick']),
        )

    def test_departure_arrival__multiple_runs(self, logger: Logger, logger2: Logger, log_collector: LogCollector,
                                              _departure_arrival_all_df):
        self.setup_departure_arrival_1(logger)
        self.setup_departure_arrival_2(logger)
        self.setup_departure_arrival_3(logger)
    ...
    def test_get_faults(self, _faults_df, logger,
                        _platform_blocked_fault_configuration: PlatformBlockedFaultConfiguration,
                        _track_blocked_fault_configuration: TrackBlockedFaultConfiguration,
                        _track_speed_limit_fault_configuration: TrackSpeedLimitFaultConfiguration,
                        _schedule_blocked_fault_configuration: ScheduleBlockedFaultConfiguration,
        ...
        self.setup_faults(logger, _platform_blocked_fault_configuration, _track_blocked_fault_configuration,
                          _track_speed_limit_fault_configuration, _schedule_blocked_fault_configuration,
                          _train_prio_fault_configuration, _train_speed_fault_configuration)
        _faults_df['fault_id'] = [_platform_blocked_fault_configuration.id,
