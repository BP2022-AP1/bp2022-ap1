...
class DataProvider:
    ...
    def _start_index(self, start: datetime) -> int:
        ...
        return max(
            self._available_index_timestamps.index([
                timestamp for timestamp in self._available_index_timestamps
    ...
    def _start_iteration_index(self, start: datetime) -> int:
        ...
        """
        return [
            i for i in range(len(self._loaded_data))
            if self._loaded_data[i][0] >= start
        ][0]
    ...
    def _load_data_between(self, start: datetime, end: datetime):
        ...
        for index_timestamp in self._available_index_timestamps[start_index:end_index + 1]:
            if index_timestamp in self._loaded_data_index_timestamps:
                continue
            self._load_data(index_timestamp)

    def data_available(self, start: datetime, end: datetime) -> tuple[bool, tuple[datetime | None, datetime | None]]:
    ...
    def get_data(self, start: datetime, end: datetime) -> list[tuple[datetime, float]]:
        ...
        return [
            self._loaded_data[i] for i in range(
